// ABOUTME: Command to enable GongShow system-wide.
...
var enableCmd = &cobra.Command{
	Use:     "enable",
	GroupID: GroupConfig,
	Short:   "Enable GongShow system-wide",
	Long: `Enable GongShow for all agentic coding tools.
...
Environment overrides:
  GONGSHOW_DISABLED=1  - Disable for current session only
...
func runEnable(cmd *cobra.Command, args []string) error {
	if err := state.Enable(Version); err != nil {
		return fmt.Errorf("enabling GongShow: %w", err)
	}

	fmt.Printf("%s GongShow enabled\n", style.Success.Render("✓"))
	fmt.Println()
	fmt.Println("GongShow will now:")
